...
- **지역 제한 (2026-08-27 팀 결정)**: 커스텀 코스는 **강원도 전체**에서만 생성 가능 (관광콘텐츠 공모전 취지 고려). 강원 본토 박스(위도 36.9~38.7, 경도 127.4~129.5) + 철원군 전용 박스(위도 37.95~38.45, 경도 127.0~127.45)의 합집합으로 프론트(지도 클릭 시)/백엔드(`CourseWaypointCreate`) 양쪽에서 검증. 박스 1개로는 철원군을 담으면 서울까지 같이 들어와버려서 2개로 분리함
